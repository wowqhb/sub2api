  ...
  <div :class="flat ? '' : 'card overflow-hidden'">
    ...
    >
      ...
      >
        {{
          ipGeoBatchLoading
            ? t("usage.ipGeo.batchFetching")
            : t("usage.ipGeo.batchFetch")
        }}
      </button>
    </div>
    <div class="overflow-auto">
      ...
      >
        ...
        <template #cell-reasoning_effort="{ row }">
          <div
            v-if="hasReasoningEffortMapping(row)"
          ...
          >
            ...
            <div class="text-gray-500 dark:text-gray-400">
              ...
              >{{ formatReasoningEffort(row.upstream_reasoning_effort) }}
            </div>
          </div>
          <span
            v-else
        ...
        <template #cell-tokens="{ row }">
          ...
          <div v-if="isImageUsage(row)" class="flex items-center gap-1.5">
            ...
            <span class="text-gray-400"
              >({{ formatImageBillingSize(row, t) }})</span
            >
          </div>
          <!-- Token 请求 -->
          <div v-else class="flex items-center gap-1.5">
            <div class="space-y-1 text-sm">
              ...
              <div
                v-if="hasImageInputTokens(row)"
                class="flex items-center gap-2"
              >
                <div class="inline-flex items-center gap-1">
                  <svg
                    class="h-3.5 w-3.5 text-fuchsia-500"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                  ...
                  <span
                    class="font-medium text-fuchsia-600 dark:text-fuchsia-400"
                    >{{ row.image_input_tokens.toLocaleString() }}</span
                  >
                </div>
              </div>
              <div
              ...
              >
                <div class="inline-flex items-center gap-1">
                  <svg
                    class="h-3.5 w-3.5 text-pink-500"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
        ...
        <template #cell-cost="{ row }">
          <div class="text-sm">
            ...
            <div
              ...
              class="mt-0.5 text-[11px] text-orange-500 dark:text-orange-400"
            >
          ...
          <div class="flex items-stretch gap-2">
                ...
                row.first_token_ms != null
                  ? [
                      ...
                      LATENCY_BAR_FROM_CLASSES[
                        firstTokenSeverity(row.first_token_ms)
                      ],
                      LATENCY_BAR_TO_CLASSES[
                        durationSeverity(row.duration_ms ?? 0)
                      ],
                    ]
            ...
            >
              ...
              <span
                ...
                :class="
                  LATENCY_TEXT_CLASSES[firstTokenSeverity(row.first_token_ms)]
                "
                >{{ formatDuration(row.first_token_ms) }}</span
              >
              <span v-else class="text-gray-400 dark:text-gray-500">-</span>
              <span class="text-gray-400 dark:text-gray-500">{{
                t("usage.latencyDuration")
              }}</span>
              <span
                class="font-medium tabular-nums"
                :class="
                  LATENCY_TEXT_CLASSES[durationSeverity(row.duration_ms ?? 0)]
                "
        ...
        <template #cell-request_id="{ row }">
          <div
            v-if="row.request_id"
            class="flex max-w-[160px] items-center gap-1.5"
          >
            <span
        ...
        <template #cell-upstream_request_id="{ row }">
          ...
          >
            ...
            <button
              ...
              class="shrink-0 rounded p-0.5 text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600 dark:hover:bg-dark-700 dark:hover:text-gray-300"
              :class="
                copiedRequestId === row.upstream_request_id
                  ? 'text-green-500 hover:text-green-500'
                  : ''
              "
              :title="
            ...
            >
              <Icon
                :name="
                  copiedRequestId === row.upstream_request_id ? 'check' : 'copy'
                "
                size="sm"
                class="h-3.5 w-3.5"
              />
          ...
          <div>
            ...
            <div
              v-if="
                ...
                !hasImageInputTokens(tokenTooltipData)
              "
            ...
            <div
              v-if="
                tokenTooltipData &&
                hasImageInputTokens(tokenTooltipData) &&
                textInputTokens(tokenTooltipData) > 0
              "
              class="flex items-center justify-between gap-4"
            >
              <span class="text-gray-400">{{
              ...
              <span class="font-medium text-white">{{
                textInputTokens(tokenTooltipData).toLocaleString()
              }}</span>
            </div>
            <div
              v-if="tokenTooltipData && hasImageInputTokens(tokenTooltipData)"
              class="flex items-center justify-between gap-4"
            >
              <span class="text-gray-400">{{
                t("usage.imageInputTokens")
              }}</span>
              <span class="font-medium text-fuchsia-300">{{
                tokenTooltipData.image_input_tokens.toLocaleString()
              }}</span>
            </div>
            <div
              v-if="
                ...
                !hasImageOutputTokens(tokenTooltipData)
              "
              class="flex items-center justify-between gap-4"
            >
              <span class="text-gray-400">{{
            ...
            <div
              v-if="
                tokenTooltipData &&
                hasImageOutputTokens(tokenTooltipData) &&
                textOutputTokens(tokenTooltipData) > 0
              "
              class="flex items-center justify-between gap-4"
            >
              <span class="text-gray-400">{{
            ...
            <div
              v-if="tokenTooltipData && hasImageOutputTokens(tokenTooltipData)"
              class="flex items-center justify-between gap-4"
            >
              <span class="text-gray-400">{{
        ...
        <div class="space-y-1.5">
          ...
          <div class="mb-2 border-b border-gray-700 pb-1.5">
            ...
            >
              ...
              <span class="font-medium text-white"
                >¥{{ tooltipData.input_cost.toFixed(8) }}</span
              >
            </div>
            <div
            ...
            >
              ...
              <span class="font-medium text-fuchsia-300"
                >¥{{ tooltipData.image_input_cost.toFixed(8) }}</span
              >
            </div>
            <div
            ...
            >
              ...
              <span class="font-medium text-white"
                >¥{{ tooltipData.output_cost.toFixed(8) }}</span
              >
            </div>
            <div
            ...
            >
              ...
              <span class="font-medium text-pink-300"
                >¥{{ tooltipData.image_output_cost.toFixed(8) }}</span
              >
            </div>
            ...
            <template
              v-if="
                tooltipData &&
                !isImageUsage(tooltipData) &&
                (!tooltipData.billing_mode ||
                  tooltipData.billing_mode === BILLING_MODE_TOKEN)
              "
            >
              <div
                v-if="tooltipData && textInputTokens(tooltipData) > 0"
                class="flex items-center justify-between gap-4"
              >
                <span class="text-gray-400">{{
              ...
              <div
                v-if="tooltipData && hasImageInputTokens(tooltipData)"
                class="flex items-center justify-between gap-4"
              >
                <span class="text-gray-400">{{
                  t("usage.imageInputTokenPrice")
                }}</span>
                <span class="font-medium text-fuchsia-300"
                  >{{
                    formatTokenPricePerMillion(
                      tooltipData.image_input_cost ?? 0,
                      tooltipData.image_input_tokens,
                    )
                  }}
                  {{ t("usage.perMillionTokens") }}</span
                >
              ...
              <div
                v-if="
                  ...
                  textOutputTokens(tooltipData) > 0
                "
                class="flex items-center justify-between gap-4"
              >
                <span class="text-gray-400">{{
              ...
              <div
                v-if="tooltipData && hasImageOutputTokens(tooltipData)"
                class="flex items-center justify-between gap-4"
              >
                <span class="text-gray-400">{{
                ...
                <span class="font-medium text-pink-300"
                  >{{
                    formatTokenPricePerMillion(
                      tooltipData.image_output_cost ?? 0,
                      tooltipData.image_output_tokens,
                    )
                  }}
                  {{ t("usage.perMillionTokens") }}</span
                >
            ...
            <template v-else-if="tooltipData && isImageUsage(tooltipData)">
              <div class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-white"
                  >{{ tooltipData.image_count }}{{ t("usage.imageUnit") }}</span
                >
              </div>
              <div class="flex items-center justify-between gap-4">
                <span class="text-gray-400">{{
              ...
              <div
                v-if="formatImageSizeBreakdown(tooltipData)"
                class="flex items-center justify-between gap-4"
              >
                <span class="text-gray-400">{{
              ...
              <div class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-sky-300"
                  >¥{{ imageUnitPrice(tooltipData).toFixed(8) }}</span
                >
              </div>
              <div class="flex items-center justify-between gap-4">
                <span class="text-gray-400">{{
                ...
                <span class="font-medium text-white"
                  >¥{{
                    tooltipData.total_cost?.toFixed(8) || "0.00000000"
                  }}</span
                >
              </div>
            </template>
            ...
            >
              ...
              <span class="font-medium text-white"
                >¥{{ tooltipData.cache_creation_cost.toFixed(8) }}</span
              >
            </div>
            <div
            ...
            >
              ...
              <span class="font-medium text-white"
                >¥{{ tooltipData.cache_read_cost.toFixed(8) }}</span
              >
            </div>
          </div>
          <!-- Rate and Summary -->
          ...
          <div class="flex items-center justify-between gap-6">
            ...
            <span class="font-medium text-white"
              >¥{{ tooltipData?.total_cost?.toFixed(8) || "0.00000000" }}</span
            >
          </div>
          <div class="flex items-center justify-between gap-6">
            <span class="text-gray-400">{{ t("usage.userBilled") }}</span>
            <span class="font-semibold text-green-400"
              >¥{{ tooltipData?.actual_cost?.toFixed(8) || "0.00000000" }}</span
            >
          </div>
          ...
          <template v-if="showAccountBilling">
            ...
            >
              ...
              <span class="font-semibold text-blue-400"
                >{{
                  formatMultiplier(tooltipData?.account_rate_multiplier ?? 1)
                }}x</span
              >
            </div>
            <div class="flex items-center justify-between gap-6">
...
const hasReasoningEffortMapping = (row: AdminUsageLog): boolean => {
  ...
  return (
    requested !== "" &&
    forwarded !== "" &&
    !reasoningEffortValuesEqual(requested, forwarded)
  );
};

const sentUpstreamModel = (row: AdminUsageLog): string =>
  row.upstream_model?.trim() || row.model?.trim() || "";

const normalizeModelVariant = (model: string): string =>
  model
    .trim()
    .toLowerCase()
    .replace(/-latest$/, "")
...
const isLikelyModelVariant = (row: AdminUsageLog): boolean => {
  ...
  return (
    sent !== "" &&
    response !== "" &&
    normalizeModelVariant(sent) === normalizeModelVariant(response)
  );
};
...
const copyRequestId = (requestId: string) =>
  copyIdentifier(requestId, t("admin.usage.requestIdCopied"));

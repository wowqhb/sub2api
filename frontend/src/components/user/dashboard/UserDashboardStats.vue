    ...
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
      ...
      >
        <div class="flex items-center justify-between">
          ...
          <span
            class="font-mono text-sm text-purple-600 dark:text-purple-400"
            :title="t('dashboard.actual')"
          >
        ...
        <div class="mt-2 space-y-1 text-xs">
          <div class="flex items-center justify-between">
            ...
            <span class="font-mono text-gray-900 dark:text-white"
              >¥{{ formatCost(item.today_actual_cost) }}</span
            >
          </div>
          <div class="flex items-center justify-between">
            <span class="text-gray-500 dark:text-gray-400">{{
                  ...
                  {{
                    t("dashboard.platformQuota.resetsAt", {
                      time: formatResetTime(
                        quotaVal(item.quota, `${w}_window_resets_at`) as string,
                      ),
                    })
                  }}
                </p>
...
const platformCount = computed(
  () => platformCards.value.filter((c) => !c.isOther).length,
);

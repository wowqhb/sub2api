export default {
  ...
  keys: {
    ...
    bulkEdit: {
      ...
      failureHint:
        "以下密钥更新失败，可修改设置后重试。再次提交只会更新失败的密钥。",
    },
    deleteKey: "删除密钥",
    ...
    useKeyModal: {
      ...
      openai: {
        ...
        authModeDescription:
          "兼容模式保留旧版 Codex 配置；API Key Mode 用于授权客户端图片执行器。",
        authModeLegacy: "兼容模式",
      ...
      grok: {
        ...
        codexDescription:
          "配置 Codex，让 Responses API 请求通过当前 Sub2API Grok 分组发送。",
        configTomlHint:
      ...
      deepseek: {
        description:
          "通过当前 DeepSeek 分组配置 Claude Code、Codex 或 OpenCode。",
        codexDescription:
          "使用 API Key 配置 Codex，并通过当前 DeepSeek 分组发送请求。",
        codexConfigTomlHint:
          "下载下方模型目录，将两个文件保存到 Codex 配置目录后重启 Codex。",
        codexNote:
          "启动 Codex 前先导出 SUB2API_API_KEY。下载的目录只包含模型元数据，不包含 API Key。",
      },
      minimax: {
        description:
          "通过当前 MiniMax 分组配置 Claude Code、Codex 或 OpenCode。",
        codexDescription:
          "使用 API Key 配置 Codex，并通过当前 MiniMax 分组发送请求。",
        codexConfigTomlHint:
          "下载下方模型目录，将两个文件保存到 Codex 配置目录后重启 Codex。",
        codexNote:
          "启动 Codex 前先导出 SUB2API_API_KEY。下载的目录只包含模型元数据，不包含 API Key。",
      ...
      composite: {
        ...
        codexDescription:
          "使用 API Key 和当前 Composite 分组的完整模型目录配置 Codex。",
        codexConfigTomlHint:
          "下载下方模型目录，将两个文件保存到 Codex 配置目录后重启 Codex。",
        codexNote:
          "启动 Codex 前先导出 SUB2API_API_KEY；分组会根据目录中选中的模型路由请求。",
      },
      routedCodex: {
      ...
      codexModelCatalog: {
        title: "Codex 模型目录",
        description:
          "使用当前 API Key 获取目录，并保存到 config.toml 引用的路径。",
        fetch: "获取目录",
        retry: "重试",
        download: "下载目录",
  ...
  modelPlaza: {
    ...
    detail: {
      ...
      longContextDisabledNote:
        "该分组未启用长上下文阶梯计费，超阈值请求仍按基础档计费，官方阶梯仅供参考",
    },
    table: {
      model: "模型",
      input: "输入",

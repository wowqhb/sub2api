export default {
  ...
  backup: {
    ...
    imageStorage: {
      title: "异步生图对象存储",
      description:
        "开启后，异步生图接口可用，生成结果转存到对象存储，只把短链接写入 Redis。与备份共用同一套 S3 客户端，保存后立即生效，无需重启。",
      enabled: "启用异步生图",
    ...
    actions: {
      ...
      downloadParts: "下载分卷",
      downloadPartsHint:
        "请按顺序下载全部分卷后拼接 gzip 字节流：Linux/macOS 使用 cat payload.part-* > backup.sql.gz；Windows 使用 copy /b payload.part-000001+payload.part-000002 backup.sql.gz。",
      partLabel: "第 {index} 卷",
  ...
  users: {
    ...
    bulkDelete: {
      ...
      title: "删除已选用户",
      confirm:
        "确定删除已选的 {count} 个用户吗？此操作无法撤销。管理员账号无法删除。",
    ...
    restrictPublicGroupsHint:
      "开启后，该用户仅能使用下方勾选的公开分组；关闭则可使用全部公开分组。",
    ...
    platformQuota: {
      ...
      clearAllConfirm:
        '确认清空全部平台的日 / 周 / 月限额？所有平台将变为"无限额"，本地无法撤销，需要在保存前手动重填。',
      reset: {
        button: "重置该窗口",
        unavailable: "该平台未配置限额，没有可重置的用量窗口",
        confirm:
          "确认重置该用户 {platform} 平台的 {window} 用量？此操作立即生效。",
      ...
      subscriptionWarning:
        "此用户有活跃订阅，平台限额仅在余额（标准）模式下生效，订阅模式请求不受此限额约束。",
  ...
  groups: {
    ...
    form: {
      ...
      maxReasoningEffortHint:
        "仅限制客户端主动请求的 Anthropic/OpenAI 推理强度；Composite 分组按实际目标平台生效。不会为缺省请求主动开启推理。上限优先级高于推理强度映射。",
      maxReasoningEffortOverLimit: "超限访问控制",
      maxReasoningEffortOverLimitDowngrade: "超过上限时自动降档",
      maxReasoningEffortOverLimitDeny: "拒绝访问",
      maxReasoningEffortOverLimitHint:
        "设置上限后生效。自动降档会将超过上限的请求改写为上限值后转发；拒绝访问则直接返回错误。",
      reasoningEffortMappings: "推理强度映射",
      reasoningEffortMappingsHint:
        "类型和模型均可留空，表示匹配全部模型。同一类型和模型下可添加多条请求值映射，例如前缀 gpt 同时将 high、xhigh 转到 medium。转发值可选拒绝，命中对应请求值时直接返回错误。精确优先于前后缀，更长前后缀优先。",
    ...
    imagePricing: {
      ...
      batchSectionHint:
        "批量生图仅影响批量任务：结算价格会叠加批量折扣倍率，提交时冻结金额按普通生图原价 × 批量冻结价格比例计算。参考图也会产生上游输入 token 消耗，建议批量生图折扣倍率设置大于 0.5。",
    ...
    videoPricing: {
      ...
      modelOverridesDescription:
        "已填写的单元格会覆盖该模型族的平面分辨率价格。video-1.5 的 preview 与 legacy 别名共用同一模型族；留空则回退到平面分辨率价格。",
    ...
    explicitPricing: {
      ...
      description:
        "分组级 web_search（每千次）与 Voice realtime / TTS / STT 单价（USD）。留空表示未配置。",
    ...
    modelPricing: {
      title: "分组逐模型定价",
      description:
        "匹配模型后覆盖渠道和内置价格。长上下文阶梯沿用官方/预设价卡，无需再手填区间。音频可用按次层级配置 realtime、tts、stt。",
      longContext: "启用长上下文阶梯定价",
      longContextHint:
        "勾选后按渠道区间或官方预设阶梯计费；关闭后默认按第一档，账号显式开启时除外。",
      add: "添加模型价格",
    },
    voicePricing: {
      title: "Grok Voice 定价",
      description:
        "分组级 Voice realtime / TTS / STT 单价（USD）。留空表示未配置。",
    ...
    peakRate: {
      ...
      peakMultiplier: "高峰倍率",
      multiplierHint:
        "作用于 token 计费倍率；token 计费的图片 token 同样适用，0 表示高峰 token 请求按 0 倍计费",
    },
    profitControl: {
      enable: "启用利润控制",
      enabledHint:
        '调度时仅允许"账号倍率 ≤ 请求实际下游倍率 ×（1 − 最低毛利率 − 安全缓冲）"的账号进入候选池；账号倍率可手工维护或由探测同步，既有排序、粘性与熔断在合格账号间照常工作。图片/视频调度暂不参与。',
      disabledHint:
        "关闭后调度不做利润过滤，账号倍率高于下游倍率的账号也会被选中，可能产生亏损请求。",
      minMargin: "最低毛利率（%）",
    ...
    codexModelsManifest: {
      ...
      disabledHint:
        "未启用：普通列表使用本地映射或默认模型；Codex 优先使用本地目录，无本地目录时由调度器选账。",
      accounts: "选定账号",
      searchPlaceholder: "搜索账号（当前分组内 OpenAI 账号）",
      searchEmpty: "未找到匹配账号",
      fallback: "选定账号全部不可用时回退调度器",
      fallbackHint:
        "关闭时返回 503 / 上游错误；开启时回退到现有调度器选账路径。",
    ...
    compositeRoutes: {
      ...
      upstreamModel: "上游模型",
      upstreamModelHint:
        "留空表示透传原始请求模型：前缀匹配下每个命中模型各自原样转发（如 deepseek-v4-flash、deepseek-v4-pro 分别转发）；填写则所有命中请求都固定转发该模型。",
      notes: "备注",
      enabled: "启用",
      preview: "预览",
      matched: "已匹配",
    ...
    openaiLive: {
      ...
      unsupportedMessage:
        "当前 Sub2API 服务端无法生成 Live 所需的设备证明，即使开启也不能使用。是否仍然开启？",
      enableAnyway: "仍然开启",
    },
    openaiFast: {
      ...
      free: "免费 Fast",
      freeHint:
        "该分组的 Fast 请求仍使用 priority 档位，但客户实际费用按同一请求的 Standard 价格计算。",
    },

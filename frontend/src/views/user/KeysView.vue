    ...
    <TablePageLayout>
        ...
        <div class="flex flex-col gap-3">
          ...
          <EndpointPopover
            v-if="
              ...
              (publicSettings?.custom_endpoints?.length ?? 0) > 0
            "
      ...
      <template #table>
        <DataTable
          ...
          :selection-label="
            (key: ApiKey) => t('keys.bulkEdit.selectKey', { name: key.name })
          "
        ...
        <fieldset v-if="!showEditModal" data-tour="key-form-provider">
          ...
          <div class="grid grid-cols-2 gap-2 sm:grid-cols-4">
            <label
              ...
              :class="
                provider.count === 0 ? 'cursor-not-allowed' : 'cursor-pointer'
              "
            >
              <input
                type="radio"
              ...
              >
                <span
                  class="flex h-8 items-center justify-center gap-1.5"
                  aria-hidden="true"
                >
                  <span
...
const handleSelectionChange = (ids: Array<string | number>) => {
  ...
  selectedIds.value = [...new Set(ids.map(Number))].filter((id) =>
    visibleIds.has(id),
  );
};
...
const createProviderOptions = computed(() =>
  KEY_GROUP_PROVIDERS.map((value) => ({
    ...
    count: groups.value.filter(
      (group) => getKeyGroupProvider(group.platform) === value,
    ).length,
  })),
);
...
watch(
  [showCreateModal, createProviderOptions],
  ([isOpen, providers], [wasOpen]) => {
    if (!isOpen) return;
    if (
      !wasOpen ||
      !providers.some(
        (provider) =>
          provider.value === createProvider.value && provider.count > 0,
      )
    ) {
      selectCreateProvider(
        providers.find((provider) => provider.count > 0)?.value ?? "anthropic",
      );
    }
    if (
      !formGroupOptions.value.some(
        (group) => group.value === formData.value.group_id,
      )
    ) {
      formData.value.group_id = null;
    }
...
const openGroupSelector = (key: ApiKey) => {
  if (groupSelectorKeyId.value === key.id) {
    ...
    if (buttonEl) {
      ...
      const left = Math.max(
        8,
        Math.min(rect.left, window.innerWidth - dropdownEstWidth - 8),
      );
    ...
    if (key.status !== updatedKey.status) {
      ...
      if (selectedKey.value?.id === key.id) {
        formData.value.status =
          updatedKey.status === "active" ? "active" : "inactive";
      }
    }
  } catch (error: any) {
    const errorMsg =

  ...
  <AppLayout>
      ...
      <template v-else>
        ...
        <template v-if="paymentPhase === 'paying'">
          <PaymentStatusPanel
            ...
            :mobile-alipay-deep-link="
              paymentState.alipayMobilePrecreateDeepLink
            "
            @done="onPaymentDone"
                ...
                <div class="space-y-2 text-sm">
                  ...
                  >
                    ...
                    <span
                      ...
                      >{{ formatSelectedPaymentAmount(totalAmount) }}</span
                    >
                  </div>
                  <div
              ...
              >
                <div class="space-y-2 text-sm">
                  ...
                  >
                    ...
                    <span
                      class="text-lg font-bold text-primary-600 dark:text-primary-400"
                      >{{ formatSelectedPaymentAmount(subTotalAmount) }}</span
                    >
                  </div>
                </div>
              </div>
                    ...
                    <div class="min-w-0 flex-1">
                      ...
                      >
                        ...
                        <span v-if="subscriptionHasPeakRate(sub)"
                          >{{ t("payment.planCard.peakRate") }}:
                          {{ subscriptionPeakRateLabel(sub) }}</span
                        >
                        <span
                          v-if="
        ...
        >
          <div class="flex flex-col items-center gap-3">
            ...
            <div
              ...
              class="markdown-body w-full overflow-x-auto break-words"
    ...
    <Teleport to="body">
      <Transition name="modal">
        ...
        >
          <div
            class="relative flex max-h-full w-full max-w-lg flex-col rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl dark:border-dark-700 dark:bg-dark-900"
          >
            <!-- Close button -->
            <button
            ...
            <h3
              class="mb-4 shrink-0 text-lg font-semibold text-gray-900 dark:text-white"
            >
...
function subscriptionHasPeakRate(sub: {
  group?: PeakRateFields | null;
}): boolean {
  return hasPeakRate(sub.group);
}

function subscriptionPeakRateLabel(sub: {
  group?: PeakRateFields | null;
}): string {
...
const renderedHelpText = computed(() =>
  DOMPurify.sanitize(
    marked.parse(checkout.value.help_text || "", {
      async: false,
      gfm: true,
      breaks: false,
    }),
...
const visibleMethods = computed(() =>
  getVisibleMethods(checkout.value.methods),
);
...
const selectedLimit = computed(
  () => visibleMethods.value[selectedMethod.value],
);
...
function subscriptionPaymentAmountForCurrency(
  value: number,
  currency: string,
): number {
...
const methodOptions = computed<PaymentMethodOption[]>(() =>
  enabledMethods.value.map((type) => {
    ...
    return {
      ...
      available:
        ml?.available !== false && amountFitsMethod(validAmount.value, type),
    };
  }),
);
...
const amountError = computed(() => {
  ...
  if (
    !enabledMethods.value.some((m) => amountFitsMethod(validAmount.value, m))
  ) {
  ...
  if (ml) {
    if (ml.single_min > 0 && validAmount.value < ml.single_min)
      return t("payment.amountTooLow", {
        min: formatSelectedPaymentAmount(ml.single_min),
      });
    if (ml.single_max > 0 && validAmount.value > ml.single_max)
      return t("payment.amountTooHigh", {
        max: formatSelectedPaymentAmount(ml.single_max),
      });
  }
  return "";
});
...
const subFeeAmount = computed(() => {
  ...
  return ceilPaymentAmount(
    (subPaymentAmount.value * feeRate.value) / 100,
    selectedCurrency.value,
  );
});

const subTotalAmount = computed(() => {
  if (feeRate.value <= 0 || subPaymentAmount.value <= 0)
    return subPaymentAmount.value;
  return roundPaymentAmount(
    subPaymentAmount.value + subFeeAmount.value,
    selectedCurrency.value,
  );
});
...
function subscriptionTotalAmountForCurrency(
  ...
  const fee = ceilPaymentAmount(
    (paymentAmount * feeRate.value) / 100,
    currency,
  );
  ...
  return enabledMethods.value.map((type) => {
    ...
    return {
      ...
      available:
        ml?.available !== false &&
        amountFitsMethod(
          subscriptionTotalAmountForCurrency(price, currency),
          type,
        ),
    };
  });
...
function shouldFallbackToDesktopQr(
  ...
  const normalizedMethod =
    normalizeVisibleMethod(paymentMethod) || paymentMethod;
  const reason =
    typeof err === "object" &&
    err &&
    "reason" in err &&
...
async function resumeWechatPaymentFromQuery() {
  ...
  if (resume.orderType === "subscription" && resume.planId) {
    selectedPlan.value =
      checkout.value.plans.find((plan) => plan.id === resume.planId) ?? null;
  }
    ...
    if (typeof window !== "undefined") {
      ...
      if (restored) {
        ...
        const restoredMethod =
          ...
          (visibleMethods.value[restored.paymentType]
